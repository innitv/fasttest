...
export const COPY = {
  ...
  // ── Архетип D: билетный чекаут ─────────────────────────────────────
  // «Способ оплаты» единственным числом — так у билетного донора (снято с
  // живого чекаута); архетип A называет ту же секцию «Оплата».
  ...
  // ── Архетипы F и G: форма заказа со слотом и чекаут с бонусами ─────
  // Плейсхолдер выпадающего списка оплаты: у донора список всегда открывается
  // с уже выбранным значением, поэтому пустое состояние — только для темы,
  // где `default_selected` не задан.
  "payment.select_placeholder": "Выберите способ",
  ...
  /* Метка поля у карточного оформления (макет A3 Pay): она называет не
     «номер телефона» вообще, а тот номер, по которому ищут клиента
     банка. Написание бренда — та же константа, что везде. */
  "phone.label_bank": "Телефон, привязанный к Ozon Банк",
  /* Подсказка под полем у карточного оформления: она объясняет, что
     произойдёт по кнопке и когда спишутся деньги. Длиннее общей на
     строку — слот там на две (см. `PhoneGateBlock`, look="card"). */
  "phone.hint_bank":
    "Мы отправим push-уведомление. Деньги спишутся только после подтверждения в банке.",
  "a11y.phone.clear": "Очистить номер",
  "a11y.phone.checking": "Проверяем номер",

  // ══ Домашний экран устройства (S-0) ════════════════════════════════
  // Кадр — картинка целиком: приложения на нём не открываются, и читать
  // экранному диктору поимённо нечего. Одна строка объясняет, что это.
  "home.a11y": "Домашний экран устройства с приложениями",
  /* Splash приложения сервиса: на экране только знак, и назвать
     происходящее словами может лишь эта строка. */
  "a11y.app_splash_live": (app: string) => `Открывается ${app}`,
  ...
  "push.app": "Ozon Банк",
  /*
   * Правый верх баннера. У системного уведомления там стоит время
   * доставки, и до 2026-09-06 мы ставили на его место демо-пометку.
   * Решением владельца пометка с ПУША снята: баннер должен читаться
   * как системный. На экранах банка подпись «Демонстрация: платёж не
   * выполняется» остаётся — она и есть обязательная (`bank.demo_note`).
   */
  "push.time": "Сейчас",
  ...
  // Метка карточки согласия — по образцу карточки счёта: сверху служебная
  // подпись, под ней содержание. Без неё блок читается как оборванный абзац.
  "bank.consent_title": "Условия подписки",
  ...
  /*
   * Ряд действий под чеком. У банка их ЧЕТЫРЕ, и это ряд, а не одна плитка:
   * одинокая иконка посреди пустого поля читается как недорисованный экран.
   * Подписи донорские (замер живого экрана 2026-09-05); в демо все четыре
   * статичны и скрыты от скринридера — они оформление, а не действия.
   */
  "bank.tile_repeat": "Повторить",
  "bank.docs_tile": "Документы",
  "bank.tile_template": "Шаблон",
  "bank.tile_autopay": "Автоплатёж",

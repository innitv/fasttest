...
function apply(top: string, bottom: string): void {
   ...
   *
   * Тег обновляется, только если он ЕСТЬ в документе, и никогда не создаётся
   * заново. Это не защита от опечатки, а контракт эксперимента `?tc=off`
   * (см. комментарий в `index.html`): в выключенном режиме инлайн-скрипт
   * удаляет узел ещё при разборе `<head>`, и наличие узла — единственный
   * признак режима. Разбирать `tc` здесь второй раз значило бы завести второй
   * источник правды, который разъедется с первым.
   *
   * Двухзонная канва от режима не зависит: обе переменные выставлены выше в
   * любом случае. Эксперимент как раз и проверяет, дотягивается ли она до
   * системных зон, когда тег ей не мешает.
   */

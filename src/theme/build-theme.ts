...
function buildFontFaceCss(tenant: TenantConfig): string | null {
  ...
  type Face = NonNullable<TenantConfig["typography"]["fonts"]>["body"];
  const faces = [fonts.display, fonts.body]
    .filter((face): face is NonNullable<Face> => face !== null)
    ...
    .map((face) => {
      /*
       * Дескриптор веса для вариативного файла.
       *
       * Без него `@font-face` объявляет вес `normal`, и запрошенные 725/800
       * в дескриптор не попадают. Замерено на InterVariable донора Tripster
       * с негативным контролем (`font-weight:400`): Chromium ось активирует
       * и без дескриптора — рендер совпал побайтово, — а WebKit, то есть
       * движок Safari на целевом iPhone, дорисовывает поверх настоящего
       * начертания синтетический жир: 475.36 / 482.52 px против 452.03 /
       * 459.19 с диапазоном. Заголовки донора набраны восьмисотым, и на
       * телефоне они выходили толще донорских.
       */
      const weight = face.weight_range
        ? `font-weight:${face.weight_range.min} ${face.weight_range.max};`
        : "";
      return `@font-face{font-family:"${face.family}";src:url("${face.src}") format("${fontFormat(face.src)}");${weight}font-display:swap;}`;
    });

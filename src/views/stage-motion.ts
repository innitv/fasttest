 ...
 *
 * ТЕМП берётся у системы, ФОРМА — у iOS-кривой шитов.
 *
 * Темп: у системных пресетов iOS (`smooth`, `snappy`, `bouncy`) длительность
 * 0.5 с, и Apple определяет её как «перцептивную, приблизительно равную
 * времени затухания» (docs `Animation.spring(duration:bounce:)`, WWDC23
 * «Explore SwiftUI animation»). Шкала ниже — 0.3–0.46, то есть системный
 * порядок, а не «пободрее».
 *
 * Форма: tween по `cubic-bezier(0.32, 0.72, 0, 1)`, а не пружина Motion.
 * Пружину пробовали 2026-08-22 и откатили замером: у `visualDuration` хвост
 * доигрывает ПОСЛЕ названного времени, и лист, прошедший 90% пути за 240 мс,
 * доводил последние 26 px ещё 300 мс — движение читалось как рывок с
 * подползанием. У кривой хвоста нет: длительность и есть длительность.
 * Пружина осталась там, где её поведение выверено на живом устройстве, —
 * пуш-баннер.
 *
 * История: до 2026-08-22 шкала была 220–340 мс — та же кривая, но темп в
 * 1.5–2 раза быстрее системы; владелец увидел это глазом раньше, чем показал
 * замер. Пружина пуш-баннера (ζ≈0.65, затухание ~0.5 с) была единственным
 * местом в нативном темпе — из-за чего рассинхрон и читался.
 *
...
export const SHEET_OVERLAY_SPEC: Transition = { duration: 0.44, ease: IOS_EASE };

/**
 * Затемнение под шитом. Длительность и кривая — те же, что у листа: они
 * половины одного движения, и разъезд читается сразу. Проверено 2026-08-22:
 * при затемнении 0.3 против листа 0.44 фон гас к 290 мс, а лист в этот момент
 * не прошёл и трети обратного пути — уезжал вниз по уже светлому экрану.
 *
 * Раньше значение стояло по месту (0.18 в двух экранах и спека листа в двух
 * других) — один и тот же элемент гас четырьмя разными способами.
 */
export const SHEET_SCRIM_SPEC: Transition = { duration: 0.44, ease: IOS_EASE };
...
function spec(type: TransitionType): Transition {
  switch (type) {
    ...
    case "sheet-up":
    case "sheet-down":
      return { duration: 0.46, ease: IOS_EASE };
    // Stack-навигация: короче листа, но не мгновенно.
    case "push-forward":
    case "push-back":
      return { duration: 0.42, ease: IOS_EASE };

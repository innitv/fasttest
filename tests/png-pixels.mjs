 ...
 *
 * Зачем вообще пиксели. Проверить «цвет нижней системной зоны совпадает с
 * цветом нижнего контента» через DOM нельзя честно: измерение цвета кромки —
 * это ровно то, что делает проверяемый код (`src/lib/page-canvas.ts`), и тест
 * повторил бы его же ошибку. Пиксель отрисованного кадра — независимый
 * источник: он показывает, что реально увидел бы глаз.
 *

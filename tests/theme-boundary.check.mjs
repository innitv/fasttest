...
const OUR_SURFACES = new Set([
  ...
  /*
   * Домашний экран устройства — третья сторона, системная: обои, плитки
   * приложений и док принадлежат телефону, а не подрядчику и не банку.
   * Палитра объявлена в файле как `--os-*` и заливки плиток — по той же
   * причине, что донорский градиент у экрана успеха: это КАРТИНКА
   * системного окружения, а не поверхность, темизируемая подрядчиком.
   *
   * Из темы экран читает ровно два токена — цвет и контрастную краску
   * плитки подрядчика (`--t-brand-primary`, `--t-brand-primary-on`): знак
   * приложения на чужом экране обязан быть его цвета.
   */
  "HomeScreen.tsx",
  /*
   * Фирменный знак A3 Pay: цвета частей — часть САМОГО ЗНАКА, а не
   * оформление экрана. Их нельзя ни подставить из темы, ни поправить под
   * контраст: перекрашенный логотип — уже не логотип. Палитра перенесена
   * из брендового репозитория вместе с геометрией и там же является
   * источником правды.
   */
  "BrandMarkAssembly.tsx",
]);

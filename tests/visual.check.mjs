...
const FRAMES = [
  ...
  /*
   * Страница со списком демо: она поставляется в прод по служебному пути и
   * ломается тихо — новая тема попадает в список сама, а вот раскладка и
   * подписи ничем, кроме кадра, не сторожатся.
   */
  ["launcher", "/__launcher"],
